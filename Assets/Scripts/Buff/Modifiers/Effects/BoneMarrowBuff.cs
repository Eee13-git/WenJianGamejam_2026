...
/// 骨髓应急崩解 buff 效果 — 施加瞬间：
/// 1) 施法者扣除 1/2 当前生命值；
/// 2) 玩家施放：立即清除地图内所有非 Boss 敌人，Boss 减少 1/4 当前血量；
///    敌人施放：扣除玩家及其随从（同化敌人）1/4 当前血量。
/// buff 是短时增益（效果一次性完成）。
...
{
    [Header("崩解配置")]
    [Tooltip("自身当前生命值扣除比例（0.5 = 扣一半）")]
    public float maxHealthCostRatio = 0.5f;

    [Tooltip("Boss 当前血量扣除比例（0.25 = 扣 1/4）")]
    ...
    /// <summary>施法者扣除当前生命值</summary>
    private void ApplyHealthCost(GameObject target)
    {
        var stats = target.GetComponent<PlayerStats>();
        if (stats != null)
        {
            float cost = stats.CurrentHealth * maxHealthCostRatio;
            stats.TakeDamage(cost);

            Debug.Log($"[BoneMarrow] 自身扣血 {cost:F0}（当前血的 {maxHealthCostRatio * 100f:F0}%）");
            return;
        }
        ...
        if (health != null)
        {
            float cost = health.CurrentHealth * maxHealthCostRatio;
            health.TakeDamage(cost);
        }
    }

    /// <summary>敌人施放：扣除玩家及其随从（同化敌人）1/4 当前血量</summary>
    private void DamagePlayerAndFollowers()
    {
        ...
        if (playerStats != null)
        {
            playerStats.TakeDamage(playerStats.CurrentHealth * playerDamageRatio);
            Debug.Log($"[BoneMarrow] 扣除玩家 {playerDamageRatio * 100f:F0}% 当前血");
        }
        ...
        foreach (var enemy in enemies)
        {
            ...
            if (health != null)
            {
                health.TakeDamage(health.CurrentHealth * playerDamageRatio);
                followerCount++;
            }
        }
        if (followerCount > 0)
            Debug.Log($"[BoneMarrow] 扣除 {followerCount} 个随从 {playerDamageRatio * 100f:F0}% 当前血");
    }
    ...
    private void ClearEnemies(GameObject caster)
    {
        ...
        foreach (var enemy in enemies)
        {
            ...
            // Boss：扣 1/4 当前血量（走正常伤害流程，可能触发阶段切换）
            if (enemy.GetComponent<BossCore>() != null)
            {
                var bossHealth = enemy.Health;
                if (bossHealth != null)
                {
                    float bossDamage = bossHealth.CurrentHealth * bossDamageRatio;
                    bossHealth.TakeDamage(bossDamage);
                    Debug.Log($"[BoneMarrow] Boss 扣除 {bossDamage:F0} 血（当前血的 {bossDamageRatio * 100f:F0}%）");
                }
                continue;
            }
